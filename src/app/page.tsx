...
// Screen bridge LAN para la vista en vivo (sin auth JWT, 100% opt-in).
const SCREEN_BRIDGE_URL = (process.env.NEXT_PUBLIC_SCREEN_BRIDGE_URL || "http://localhost:8100").replace(/\/$/, "");
...
interface User {
  ...
  role: Role;
  created_at?: string;
  workspace: { id: number; name: string; owner_user_id: number; bridge_url?: string | null };
}
...
function DeviceCard({ device, accounts, activeRun, token, onChanged, canManage, canRunTasks, onRevoke, bridgeUrl, liveActive, onLiveToggle }: { device: Device; accounts: SocialAccount[]; activeRun?: TaskRun; token: string; onChanged: () => void; canManage: boolean; canRunTasks: boolean; onRevoke: (id: number) => void; bridgeUrl: string; liveActive: boolean; onLiveToggle: (open: boolean) => void }) {
...
function MobileAppCard({ token }: { token: string }) {
  ...
  const createLink = async () => {
    ...
    try {
      const result = await request<{ path: string; expires_at: string }>("/api/devices/app/install-link", token, { method: "POST" });
      setLinkPath(result.path);
      setExpiresAt(result.expires_at);
      setQrDataUrl(await QRCode.toDataURL(`${API}${result.path}`, { width: 240, margin: 1 }));
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : "No se pudo generar el link de descarga"); } finally { setBusy(false); }
  };
  return <section className="cc-card"><div className="cc-card-heading"><div><p className="cc-eyebrow cc-eyebrow-accent">APP MÓVIL</p><h3>Instalar SouthFarm</h3><p className="cc-card-subtitle">Descarga autenticada de la app. El link vence a los 15 minutos; generá otro cuando necesites.</p></div></div><div className="cc-pairing-actions"><button className="cc-button cc-button-primary" onClick={() => void createLink()} disabled={busy}>{busy ? "Generando…" : qrDataUrl ? "Generar otro QR" : "Generar QR de descarga"}<span>↓</span></button>{linkPath && <a className="cc-button cc-button-ghost" href={`${API}${linkPath}`}>Descargar APK</a>}</div>{qrDataUrl && <div className="cc-pairing-result"><div className="cc-pairing-qr"><img src={qrDataUrl} alt="QR para descargar la app en el teléfono" /><span>Escanealo desde el teléfono</span></div><small>El link vence {fullDate(expiresAt)}.</small></div>}{message && <p className="cc-inline-message is-error">{message}</p>}</section>;
}

function FleetPage({ devices, accounts, runs, token, onChanged, canManageDevices, canRunTasks, bridgeUrl }: { devices: Device[]; accounts: SocialAccount[]; runs: TaskRun[]; token: string; onChanged: () => void; canManageDevices: boolean; canRunTasks: boolean; bridgeUrl: string }) {
  ...
  return <div className="cc-page-stack"><section className="cc-section-intro"><div><p className="cc-eyebrow cc-eyebrow-accent">DEVICE CONTROL</p><h2>Tu flota, a la vista.</h2><p>La conexión y la ejecución se muestran como estados independientes.</p></div><div className="cc-intro-stats"><strong>{devices.filter((device) => device.online).length}<small>online</small></strong><span>/</span><strong>{devices.length}<small>activos</small></strong></div></section><DevicePairingCard token={token} canManage={canManageDevices} /><MobileAppCard token={token} />{devices.length ? <div className="cc-device-grid">{devices.map((device) => <DeviceCard key={device.id} device={device} accounts={accounts} activeRun={runs.find((run) => run.device_id === device.id && ["pending", "running", "paused"].includes(run.status))} token={token} onChanged={onChanged} canManage={canManageDevices} canRunTasks={canRunTasks} onRevoke={(id) => void revoke(id)} bridgeUrl={bridgeUrl} liveActive={liveDeviceIds.includes(device.id)} onLiveToggle={(open) => setLiveDeviceIds((ids) => open ? [...ids, device.id] : ids.filter((id) => id !== device.id))} />)}</div> : <section className="cc-card"><EmptyState title="No hay dispositivos registrados" detail="Vinculá un Android para que aparezca acá." /></section>}</div>;
}

function resolveProfilePicUrl(url: string) {
...
function AccountAvatar({ account }: { account: SocialAccount }) {
  ...
  const resolved = account.profile_pic_url ? resolveProfilePicUrl(account.profile_pic_url) : null;
  const src = resolved && resolved !== failedUrl ? resolved : null;
  return <div className="cc-account-avatar" style={{ color: info.color, background: `${info.color}15` }}>{src ? <img src={src} alt={account.username} onError={() => setFailedUrl(src)} /> : account.username.charAt(0).toUpperCase()}</div>;
}
...
function SettingsPage({ user, token, health, onLogout, onChanged }: { user: User; token: string; health: ApiHealth; onLogout: () => void; onChanged: () => void }) {
  const canManage = user.role === "owner" || user.role === "admin";
  const [bridgeUrl, setBridgeUrl] = useState(user.workspace.bridge_url || "");
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState("");
  const saveBridge = async () => {
    setBusy(true); setMessage("");
    try {
      await request("/api/team/workspace", token, { method: "PATCH", body: JSON.stringify({ bridge_url: bridgeUrl.trim() }) });
      setMessage("Dirección de transmisión guardada.");
      onChanged();
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : "No se pudo guardar la dirección"); } finally { setBusy(false); }
  };
  return <div className="cc-page-stack"><section className="cc-section-intro"><div><p className="cc-eyebrow cc-eyebrow-accent">WORKSPACE SETTINGS</p><h2>Configuración del centro.</h2><p>Datos de tu cuenta y del workspace actual.</p></div></section><section className="cc-card cc-settings-card"><div className="cc-settings-profile"><div className="cc-avatar cc-avatar-large">{user.name.charAt(0).toUpperCase()}</div><div><h3>{user.name}</h3><p>{user.email}</p><span className={`cc-role-pill role-${user.role}`}>{user.role}</span></div></div><div className="cc-settings-grid"><div><span>Workspace</span><strong>{user.workspace.name}</strong></div><div><span>Workspace ID</span><strong>#{user.workspace.id}</strong></div><div><span>Permiso actual</span><strong>{user.role}</strong></div><div><span>Ruta pública</span><strong className={`cc-api-value cc-health-text-${health.state}`}><span className="cc-health-dot" /> {healthLabel(health)}</strong></div></div><button className="cc-button cc-button-danger" onClick={onLogout}>Cerrar sesión</button></section><section className="cc-card"><div className="cc-card-heading"><div><p className="cc-eyebrow">LIVE VIEW</p><h3>Servidor de transmisión</h3><p className="cc-card-subtitle">URL del screen-bridge donde están conectados los celulares de este workspace. Vacío = usar la global.</p></div></div>{canManage ? <><label className="cc-form-label">Bridge URL<input className="cc-filter-input cc-input-wide" value={bridgeUrl} onChange={(event) => setBridgeUrl(event.target.value)} placeholder="https://screen-us.southfarm.tech" /></label><button className="cc-button cc-button-primary" onClick={() => void saveBridge()} disabled={busy}>{busy ? "Guardando…" : "Guardar"}</button>{message && <p className={`cc-inline-message ${message.includes("No se pudo") ? "is-error" : ""}`}>{message}</p>}</> : <p className="cc-card-subtitle">Solo owner y admin pueden cambiar esta configuración.</p>}</section></div>;
}
...
export default function Home() {
  ...
  const pageContent = page === "overview"
    ...
    : page === "publish"
      ...
      : page === "fleet"
        ? <FleetPage devices={devices} accounts={accounts} runs={runs} token={token} onChanged={() => void refresh(token)} canManageDevices={user.role === "owner" || user.role === "admin"} canRunTasks={user.role !== "viewer"} bridgeUrl={(user.workspace.bridge_url || SCREEN_BRIDGE_URL).replace(/\/$/, "")} />
      : page === "planner"
        ...
        : page === "history"
          ...
          : page === "team"
            ? <TeamPage user={user} token={token} members={members} onChanged={() => void refresh(token)} />
             : <SettingsPage user={user} token={token} health={apiHealth} onLogout={logout} onChanged={() => void refresh(token)} />;
